...
A behavior-shaping skill that runs on every chat message that is a **task instruction** (ขอให้ทำ/สร้าง/วิเคราะห์/ค้น/แก้อะไรให้ — see `AGENTS-coaching-section.md` rule 2). Its job is to evaluate the effectiveness of the user's task message and emit one 3-line 🧭 coaching block — wrapped in **its own `||...||` spoiler**, placed before the `english-coach-discord` spoiler block with one blank line between them. Plain conversational chat gets no block at all.
...
Discord chat has no `AskUserQuestion`. This skill **never stops or blocks** a reply to ask for clarification — it always answers/executes the task message inline, then appends the 🧭 coaching block. There is no pre-flight gate on this surface — coach and proceed, every time.

## When to trigger

Only on messages that are task instructions per AGENTS section rule 2. Plain conversation, small talk, or replies to the bot's own question get no 🧭 block (english-coach-discord's lines, if any, may still apply independently).
...
เลือก **gap เดียวที่คุ้มค่าที่สุด** สำหรับ block 🧭 — gap เดียวที่โค้ชดีกว่า laundry list ที่ไม่มีใครอ่าน

## Principle Catalog

ชื่อ principle อ้างอิงในบรรทัด 📐 — อิงตาม best-practices guide:
...
## Output — 3-line block (CLI-parity)

**ทุกงานที่เข้าเกณฑ์นี้ ให้ block 3 บรรทัด** (ไม่มี Format A/B แยก ไม่มี gate) — spoiler ก้อนของตัวเอง มาก่อน spoiler ของ english-coach-discord คั่นบรรทัดว่าง:

```
||🧭 **Prompt Coach:** <gap สั้นๆ ภาษาไทย>
✍️ **ลองแบบนี้:** "<ตัวอย่าง prompt ที่ดีกว่า ภาษาเดียวกับผู้ใช้>"
📐 <principle-name> — <เหตุผลไทยสั้น>||

||<spoiler ก้อน english-coach ตามมา...>||
```
...
- บรรทัด 📐 ใช้ชื่อ principle จาก catalog ตามด้วย `—` และเหตุผลสั้นภาษาไทยว่าทำไมถึงคุ้ม
- ถ้าข้อความสั่งงานดีอยู่แล้ว (ไม่มี gap ที่คุ้มค่า) → **ข้าม block นี้ไปเลย ไม่ต้องชม** (ต่างจากฉบับ marketplace ที่มี Format B ชม — บนแชตเน้นสั้น ไม่ชมทุกครั้ง)
- spoiler ก้อน english-coach ตามมาเสมอ (คั่นบรรทัดว่าง 1 เส้น) — รวมทุก section ต้อง ≤ 7 บรรทัดเนื้อหา ห้ามตาราง ห้าม heading ห้าม blockquote (`>` ใช้ร่วมกับ spoiler ไม่ได้)
...
```
🧭 **Prompt Coach:** deliverable กว้างไป — ต้องเดา content เอง
✍️ **ลองแบบนี้:** "ทำหน้า about ให้มี section ทีมงานกับ timeline บริษัท ทำให้ครบ เกินพื้นฐานไปเลย"
📐 be-explicit — ระบุสิ่งที่อยากเห็น ตัดการเดาของ bot
```
...
```
🧭 **Prompt Coach:** ไม่ได้ระบุไฟล์/ฟังก์ชันที่ต้องแก้ — bot ต้องเดาว่า "slow" อยู่ตรงไหน
✍️ **ลองแบบนี้:** "Fix the slow query in src/api/orders.ts — profile first, then fix it directly."
📐 name-targets — ระบุเป้าหมายตัดการเดาก่อนเริ่ม
```
...
→ ไม่มี gap ที่คุ้มค่า → ไม่ต้องมี block 🧭
...
- ❌ ไม่แต่ง gap ขึ้นมาเองเมื่อ prompt ดีอยู่แล้ว — ข้าม block ไปเลย
