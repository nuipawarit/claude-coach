...
A behavior-shaping skill that runs on every user chat message on Discord. Its only job is to evaluate the user's most recent message and decide whether to append a spoiler-wrapped English-learning block as a **separate follow-up message** after the bot's main reply (see `AGENTS-coaching-section.md` rule 3).

## When to trigger

Run this skill after producing the main reply. Evaluate the user's most recent chat message through the decision tree below. Based on the outcome, you either send a Format A block, a Format B block, a Format C block, or nothing at all — as a second message, wrapped once in `||...||`.
...
[3] มีตัวอักษรไทย (U+0E00–U+0E7F) อยู่ในข้อความ?
    │ yes → emit Format A (แปล + concise + optional 🎯)
    │ no
    ▼
[4] ข้อความอังกฤษมี grammar error ชัดเจน?
    ...
    │ yes → emit Format B (diff + concise + optional 🎯 + tip ไทย)
...
**Rules:**
- สูงสุด 3 บรรทัดในก้อนเดียว: แปลเต็ม / กระชับ (optional) / ยกระดับ (optional)
...
- บรรทัด 🎯 optional — ใส่เฉพาะเมื่อมี phrasing ที่เป็นธรรมชาติกว่าชัดเจน ตามด้วย `— <เหตุผลไทย>` **ข้ามถ้าคำแปลเป็นธรรมชาติอยู่แล้ว**
...
- 🎯 optional เช่นเดียวกับ Format A — ถ้าใส่ ให้แทรกก่อนบรรทัด 💡 แต่ต้องคุมรวม ≤ 5 บรรทัด (ตัดบรรทัด ✨ ออกถ้าจำเป็น)
...
- ❌ ไม่ block คำตอบหลัก — ส่งเป็นข้อความที่สองเสมอตาม AGENTS section rule 3
